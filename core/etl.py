...
def prepare_dataset(input_path: str, output_path: str, partition_cols: str) -> pd.DataFrame:
...
def transform_dataset(df: pd.DataFrame, columns: list) -> pd.DataFrame:
    df = cast_as_timestamp(df, "order_purchase_timestamp")
    return df[columns]

def cast_as_timestamp(df: pd.DataFrame, column: str) -> pd.DataFrame:
